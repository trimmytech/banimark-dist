...
Requires PHP 8.2+ with `ext-curl`, `ext-json`, PDO and the free
[ionCube Loader](https://www.ioncube.com/loaders.php) (the engine ships
encoded). Without the loader, Banimark shows a page saying exactly what to
install - the rest of your site keeps working. Restart PHP after adding it:
PHP reads `php.ini` only when it starts.

Banimark is **not on Packagist** - every install starts by telling Composer
where it lives (the `composer config` line below).
...
```bash
composer config repositories.banimark vcs https://github.com/trimmytech/banimark-dist.git
...
**No Composer?** Banimark needs no other packages, so you can skip it. Download
the release zip (`https://github.com/trimmytech/banimark-dist/archive/refs/tags/v<version>.zip`),
extract it **beside** your web root (not inside it) and rename the folder to
`banimark`. Then the entry file requires `autoload.php` instead:

```php
<?php // public_html/banimark.php
require __DIR__.'/../banimark/autoload.php';
\Banimark\Standalone\App::run(__DIR__.'/../banimark.config.php');
```

Update it from the panel's **Changelog** page - with no `composer.lock`, that is
the natural way. (Available from Banimark 0.26.)
...
| **Widget** | colour, position, logo, header line, corners, spacing, launcher icon and label, greeting (and one for out of hours), guest details, when it greets by itself, which pages carry it, reply chime, reply polling - with a live preview and a test page |
...
**From the panel, no terminal:** `/banimark/admin` → **Changelog** → *Update to
X*, then *Update the database*. Banimark downloads the release with your licence
key, checks it is exactly the file we signed, swaps it in and keeps **one**
previous version (restorable under *Previous versions*). The same page has
*Check for updates now* and *Reinstall*.

**From the command line** - name the release line you are moving to:

```bash
composer require "banimark/banimark:^0.27"   # the newest line - see CHANGELOG.md
php artisan migrate                            # Laravel; standalone: sign in to the panel once
```

Not `composer update`: on `0.x`, Composer treats each minor release as a
breaking one, so `^0.26` never installs `0.27`. `composer update` picks up fixes
within your line; `composer require` with the new line moves you up.

**Pick one way per site.** A panel update replaces the files without telling
Composer, so `composer.lock` still names the older version, and the next
`composer install` can put it back - or, from 0.26 on, rebuild the autoloader
from the old version's rules so Banimark stops loading. If your team deploys with
Composer, update with Composer. If you did use the button, record it before your
next deploy: `composer require "banimark/banimark:^X.Y"` with the version you are
now on.
