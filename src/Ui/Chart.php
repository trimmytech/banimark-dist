...
class Chart
{
    ...
    /**
     * A ring (donut): one arc per segment, the headline in the middle.
     *
     * @param array<int, array{name: string, value: int|float, color: string}> $segments
     */
    public static function ring(array $segments, string $center = '', string $centerSub = '', int $size = 128, int $thick = 14): string
    {
        $total = 0;
        foreach ($segments as $sg) {
            $total += max(0, (float) $sg['value']);
        }
        $r = ($size - $thick) / 2;
        $c = 2 * M_PI * $r;
        $mid = $size / 2;
        $out = '<svg class="ring" width="'.$size.'" height="'.$size.'" viewBox="0 0 '.$size.' '.$size.'" role="img" aria-label="'
            .self::e(implode(', ', array_map(fn ($sg) => $sg['name'].' '.self::num($sg['value']), $segments))).'">'
            .'<circle class="track" cx="'.$mid.'" cy="'.$mid.'" r="'.round($r, 2).'" fill="none" stroke-width="'.$thick.'"/>';
        $offset = 0.0;
        if ($total > 0) {
            foreach ($segments as $i => $sg) {
                $len = max(0, (float) $sg['value']) / $total * $c;
                if ($len <= 0) {
                    continue;
                }
                // a hair of gap between arcs, never more than the arc itself
                $gap = count($segments) > 1 ? min(3, $len / 3) : 0;
                $out .= '<circle class="seg-arc" cx="'.$mid.'" cy="'.$mid.'" r="'.round($r, 2).'" fill="none" stroke="'.self::e($sg['color']).'"'
                    .' stroke-width="'.$thick.'" stroke-linecap="butt" stroke-dasharray="'.round($len - $gap, 2).' '.round($c, 2).'"'
                    .' stroke-dashoffset="'.round(-$offset, 2).'" transform="rotate(-90 '.$mid.' '.$mid.')"'
                    .' style="animation-delay:'.($i * 0.08).'s"/>';
                $offset += $len;
            }
        }
        if ($center !== '') {
            $out .= '<text x="'.$mid.'" y="'.($mid + ($centerSub !== '' ? 1 : 6)).'" text-anchor="middle" font-size="'.round($size / 5.2).'" fill="currentColor">'.self::e($center).'</text>';
        }
        if ($centerSub !== '') {
            $out .= '<text x="'.$mid.'" y="'.($mid + round($size / 7)).'" text-anchor="middle" font-size="'.round($size / 12).'" fill="currentColor" opacity=".7" style="font-weight:600">'.self::e($centerSub).'</text>';
        }
        return $out.'</svg>';
    }
