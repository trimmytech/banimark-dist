...
class Installer
{
    ...
    {
        $rows = [];
        // 8.2, not 8.1: the core ships ionCube-encoded for 8.2, and an 8.2 encode
        // will not run on 8.1 at all (measured - the 8.1 loader refuses it)
        $rows[] = ['label' => 'PHP >= 8.2 (found '.PHP_VERSION.')', 'ok' => version_compare(PHP_VERSION, '8.2.0', '>='), 'required' => true, 'hint' => 'Upgrade PHP to 8.2 or newer.'];
        ...
        $rows[] = ['label' => 'a PDO driver (found: '.(implode(', ', $drivers) ?: 'none').')', 'ok' => (bool) array_intersect($drivers, \Banimark\Storage\Dialect::SUPPORTED), 'required' => true, 'hint' => 'Install pdo_mysql, pdo_pgsql or pdo_sqlite.'];
        if ($configDir !== null) {
    ...
    public static function install(array $input, string $configPath): array
    {
        ...
        try {
            $q = fn (string $i) => \Banimark\Storage\Dialect::quote(\Banimark\Storage\Dialect::of($pdo), $i);
            $st = $pdo->query("SELECT {$q('value')} FROM banimark_settings WHERE {$q('key')} = 'installed'");
        ...
        try {
            ...
            if ($slug !== '' && $slug !== 'skip' && $key !== '') {
                ...
                if (isset($presets[$slug])) {
                    ...
                    $model = trim((string) ($input['provider_model'] ?? ''));
                    // only a TESTED model; anything else starts on the default
                    if (!isset(\Banimark\Ai\ProviderPresets::MODELS[$driver][$model])) {
                        $model = $defModel;
                    }
    ...
    private static function dsn(array $input): array
    {
        ...
        $driver = ($input['driver'] ?? '') === 'pgsql' ? 'pgsql' : 'mysql';
        $host = (string) ($input['host'] ?? '127.0.0.1');
        $port = (int) ($input['port'] ?? 0) ?: ($driver === 'pgsql' ? 5432 : 3306);
