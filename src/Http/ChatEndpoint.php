...
class ChatEndpoint
{
    ...
    /**
     * The model asked for a person outside the hours the owner set, and the
     * owner said not to hand over then. The tool already told the model so;
     * the conversation stays with the assistant.
     */
    private static function wasHeldForHours(EngineResult $result): bool
    {
        foreach ($result->toolTrace as $t) {
            if (($t['tool'] ?? '') === EscalateTool::NAME && ($t['result']['status'] ?? '') === 'closed') {
                return true;
            }
        }
        return false;
    ...
    /**
     * The best name we have for whoever is chatting. A guest who typed their
     * name into the form must not become "Anonymous" in the inbox the moment
     * the conversation is handed over - which is exactly what happened while
     * this only ever looked at the signed token.
     *
     * @return string|null null = we know nothing, so leave whatever is stored
     */
    private static function visitorLabel(array $claims, array $visitor): ?string
    {
        foreach ([$claims['name'] ?? '', $visitor['name'] ?? '', $claims['email'] ?? '', $visitor['email'] ?? ''] as $candidate) {
            $candidate = trim(strip_tags((string) $candidate));
            if ($candidate !== '') {
                return mb_substr($candidate, 0, 190);
            }
        }
        return isset($claims['user_id']) ? 'user #'.$claims['user_id'] : null;
    }
    ...
    private function recordVisitor(string $sessionId, array $claims, array $visitor): void
    {
        ...
        $phone = (string) ($claims['phone'] ?? ($visitor['phone'] ?? ''));
        ...
        $phone = mb_strlen(preg_replace('/\D/', '', $phone) ?? '') >= 6 ? mb_substr($phone, 0, 40) : '';
        if ($name === '' && $email === '' && $phone === '') {
            return;
        }
        $this->store->setVisitor($sessionId, $name, $email, $phone);
    }
