...
class DoctorCommand extends Command
{
    ...
    {
        // First, and on its own: if the core cannot run here, every check below
        // would load it - and a trial encode on a PHP without the loader prints
        // its payload into the terminal instead of failing. Say what is wrong.
        if (($problem = \Banimark\CoreHealth::problem(dirname(__DIR__, 2))) !== null) {
            $this->error('  ✘ '.$problem['title']);
            $this->line('    '.$problem['message']);
            foreach ($problem['steps'] as $i => $step) {
                $this->line('    '.($i + 1).'. '.$step);
            }
            return self::FAILURE;
        }
