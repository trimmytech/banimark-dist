...
class InstallCommand extends Command
{
    ...
    private function seedProvider(): void
    {
        ...
        // a TESTED model from the list, never free text (ProviderPresets::MODELS)
        $models = array_keys(\Banimark\Ai\ProviderPresets::MODELS[$presets[$choice]['driver']] ?? []);
        $model = $models !== []
            ? (string) $this->choice('Model', $models, (int) array_search($presets[$choice]['model'], $models, true))
            : $presets[$choice]['model'];
