...
class WidgetController
{
    ...
    /** GET /banimark/widget/logo - the picture in the widget's header, or 404. */
    public function logo()
    {
        $logo = \Banimark\Http\WidgetLogo::fromSettings($this->settings());
        if ($logo === null) {
            return response('', 404);
        }
        return response($logo['bytes'], 200, \Banimark\Http\WidgetLogo::headers($logo['mime']));
    }
