...
final class PhoneHome
{
    ...
    public static function run(array $settings, string $siteUrl, callable $set, callable $forget, bool $force = false, ?callable $ping = null, ?int $now = null): ?array
    {
        ...
        // a stable per-install fingerprint, made once and kept. Not a secret and
        // not a permission - HQ only uses it to spot the SAME licence key running
        // from more installs than its plan allows (a copy), never to lock anyone.
        $installId = trim((string) ($settings['install_id'] ?? ''));
        if ($installId === '') {
            try {
                $installId = bin2hex(random_bytes(16));
            } catch (\Throwable $e) {
                $installId = substr(hash('sha256', $siteUrl.'|'.$key.'|'.$now), 0, 32);
            }
    ...
    public static function apply(array $result, callable $set, callable $forget, ?int $now = null): void
    {
        ...
        $set('license_details', (string) json_encode([
            ...
            // HQ's display name for the plan; an older HQ does not send one and
            // the licence page falls back to the slug
