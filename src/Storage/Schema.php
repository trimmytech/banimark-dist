...
class Schema
{
    ...
    {
        $driver = Dialect::of($pdo);
        $pk = Dialect::primaryKey($driver);
        $bool = Dialect::boolean($driver);
        // `key`, `value` and `sql` are reserved somewhere in every engine
        $q = fn (string $ident) => Dialect::quote($driver, $ident);
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS {$prefix}settings (
            {$q('key')} VARCHAR(80) NOT NULL,
            {$q('value')} TEXT NOT NULL
        ){$tail}");
        self::index($pdo, "{$prefix}settings_key", "{$prefix}settings", $q('key'), true);
    ...
    {
        try {
            $q = fn (string $i) => Dialect::quote(Dialect::of($pdo), $i);
            $st = $pdo->prepare("SELECT {$q('value')} FROM {$prefix}settings WHERE {$q('key')} = 'schema_version'");
            $st->execute();
            ...
            $q = fn (string $i) => Dialect::quote(Dialect::of($pdo), $i);
        $pdo->prepare("DELETE FROM {$prefix}settings WHERE {$q('key')} = 'schema_version'")->execute();
            $pdo->prepare("INSERT INTO {$prefix}settings ({$q('key')}, {$q('value')}) VALUES ('schema_version', ?)")->execute([$version]);
            return true;
        } catch (\Throwable $e) {
            return false;
        }
    ...
    {
        if (Dialect::of($pdo) !== Dialect::MYSQL) {
    ...
    {
        $exists = Dialect::of($pdo) === Dialect::PGSQL ? 'IF NOT EXISTS ' : '';
        try {
            $pdo->exec("ALTER TABLE {$table} ADD COLUMN {$exists}{$column} {$definition}");
    ...
    public static function index(\PDO $pdo, string $name, string $table, string $cols, bool $unique): void
    {
        ...
        if (Dialect::supportsCreateIndexIfNotExists(Dialect::of($pdo))) {
