...
@section('content')
    <div class="bm-card pad0">
        ...
        <div class="t-wrap">
            <table>
                <tr><th>Name</th><th>What it does</th><th>Asks the customer for</th><th>Rows</th><th>Status</th><th></th></tr>
                @forelse($rows as $r)
                    <tr style="{{ $r->enabled ? '' : 'opacity:.55' }}">
                        ...
                        <td class="row" style="gap:4px">
                            <a class="btn-ghost btn-sm" href="{{ route('banimark.admin.tools', ['edit' => $r->name]) }}#build">Edit</a>
                ...
                @empty
                    <tr><td colspan="6">{!! Chart::empty('No tools yet', 'The AI can chat, but it cannot look anything up until you build one.') !!}</td></tr>
                @endforelse
            </table>
        </div>
    </div>

    @php $e = $editing ?? null; $v = fn ($k, $d = '') => old($k, $e[$k] ?? $d); @endphp
    <div class="bm-card" id="build">
        <div class="bm-sec-h">
            <div>
                <h2>{{ $e ? 'Edit tool: '.$e['name'] : 'Build a tool' }}</h2>
                <div class="muted">{{ $e ? 'Change anything below and save — the AI uses the new version straight away.' : 'Three steps: name it, say what the AI needs to ask the customer for, then point at your data. No SQL knowledge needed — the builder writes it for you.' }}</div>
            </div>
            @if($e)<a class="btn-ghost btn-sm" href="{{ route('banimark.admin.tools') }}">Cancel — build a new one instead</a>@endif
        </div>
        <form method="post" action="{{ route('banimark.admin.tools.save') }}">
            @csrf
            @if($e)<input type="hidden" name="original_name" value="{{ $e['name'] }}">@endif
            <h3 class="bm-step">1. What is this tool?</h3>
            <div class="grid2">
                <div><label>Name <span class="muted">(letters, numbers, underscores)</span></label><input type="text" name="name" required placeholder="find_orders" value="{{ $v('name') }}"></div>
                <div><label>Most rows to return</label><input type="number" name="max_rows" value="{{ $v('max_rows', 10) }}" min="1" max="50"></div>
            </div>
            <label>Describe it in plain words — the AI reads this to know when to use it</label>
            <textarea name="description" required placeholder="Look up a customer's orders by order number or by what they bought.">{{ $v('description') }}</textarea>
            <label style="display:flex;align-items:center;gap:8px;margin-top:10px"><input type="checkbox" name="enabled" value="1" @checked(old('enabled', $e ? $e['enabled'] : true))> Tool is on (the AI may use it)</label>
            ...
            <div data-params data-prefill="{{ json_encode($e['params'] ?? []) }}"></div>
            ...
            <div data-toolbuilder data-schema-url="{{ route('banimark.admin.tools.schema') }}" style="background:var(--surface-2);border:1px solid var(--border);border-radius:12px;padding:14px 16px">
                ...
                <div class="grid2" style="margin-top:8px">
                    ...
                    <div><label>Who is chatting is identified by <span class="muted">(identity keys, comma-separated)</span></label><input type="text" name="context" value="{{ $v('context', 'user_id') }}" placeholder="user_id"></div>
                </div>
            ...
            <details style="margin-top:14px" @if($e) open @endif>
                ...
                <textarea name="sql" required placeholder="SELECT reference, status, total FROM orders WHERE reference = :reference AND user_id = :_user_id">{{ $v('sql') }}</textarea>
                <label>Columns the AI may see</label><input type="text" name="columns" required placeholder="reference, status, total" value="{{ $v('columns') }}">
            </details>
            <div style="margin-top:16px"><button type="submit">{!! Icons::get('check', 15) !!} {{ $e ? 'Validate & save changes' : 'Validate & save tool' }}</button></div>
        </form>
    </div>
