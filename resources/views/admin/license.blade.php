...
@section('content')
    ...
    @if($active)
            ...
            <div class="bm-card">
                <div class="bm-sec-h">
                    <div class="row" style="gap:10px"><span class="avatar">{!! Icons::get('license', 16) !!}</span>
                        <div><h2 style="margin:0">{{ $isTrial ? 'Free trial' : $plan['name'] }} <span class="pill active">ACTIVE</span></h2>
                ...
                <dl class="bm-dl" style="margin-top:14px">
                    <dt>Plan</dt><dd>{{ $isTrial ? 'Free trial' : $plan['name'] }}
                        @if(!$isTrial && $plan['plan'] !== '')<span class="muted mono">· {{ $plan['plan'] }}</span>@endif</dd>
        ...
        {{-- what the plan actually covers: read by the panel too, so a greyed-out
             control and this card can never say different things --}}
        {!! \Banimark\Ui\Layout::planCard($plan, $supportUrl, $supportEmail) !!}
    @else
        {{-- ============ NOT ACTIVE: trial or key ============ --}}
        {{-- how to PAY: there used to be no way at all (see Layout::renewCard) --}}
        @if($lock && !in_array($lock['reason'], ['stale', 'module'], true))
            {!! Layout::renewCard([
                'reason' => ($lock['reason'] === 'expired' && $isTrial) ? 'trial' : $lock['reason'],
                'key' => $key,
                'site' => $details['domain'] ?? request()->getHost(),
                'plan' => $isTrial ? 'Free trial' : ($plan['name'] ?? ''),
